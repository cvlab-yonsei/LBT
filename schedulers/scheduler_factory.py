...
def get_q_scheduler(config, optimizer, last_epoch):
  func = globals().get(config.scheduler.name)
  return func(optimizer, last_epoch, **config.q_scheduler.params)

def get_t_scheduler(config, optimizer, last_epoch):
  func = globals().get(config.scheduler.name)
  return func(optimizer, last_epoch, **config.q_scheduler.params)
